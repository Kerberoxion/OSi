...
typedef struct _Storage {
    Node *first;
    pthread_rwlock_t firstNodeLock;
} Storage;
...
void destroyStorage(Storage *s) {
    ...
    while(s->first){
        ...
        pthread_rwlock_destroy(&tmp->sync);
        free(tmp);

    }
    pthread_rwlock_destroy(&s->firstNodeLock);
...
void addNode(Storage *storage, char* value) {
    ...
    new_node->value[99] = '\0';
    pthread_rwlock_init(&new_node->sync, 0);
...
void* countIncPairs(void* arg) {
    ...
    while (1) {
        int err = pthread_rwlock_rdlock(&storage->firstNodeLock);
        if (err) {
            printf("pthread_rwlock_lock() failed: %s\n", strerror(err));
            return NULL;
        }
        ...
        pthread_rwlock_unlock(&storage->firstNodeLock);
        Node* next = NULL;
...
void* countEqPairs(void* arg) {
    ...
    while (1) {
        int err = pthread_rwlock_rdlock(&storage->firstNodeLock);
        if (err) {
            printf("writer: pthread_rwlock_lock() failed: %s\n", strerror(err));
            return NULL;
        }
...
void* swapNodes(void* arg) {
    Storage* storage = (Storage*)arg;
    while (1) {
        pthread_rwlock_wrlock(&storage->firstNodeLock);
...
void generateList(Storage *storage, int numNodes) {
    ...
    int err = pthread_rwlock_init(&storage->firstNodeLock, 0);
    if(err){
        printf("rwlock_iinit failed: %s\n", strerror(err));
    }
