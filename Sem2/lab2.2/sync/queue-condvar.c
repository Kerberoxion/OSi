...
void *reader(void *arg) {
	...
	while (1) {
        ...
        while(max_count == 0){
            err = pthread_cond_wait(&condvar, &lock);
            if (err) {
                printf("reader: pthread_cond_wait() failed: %s\n", strerror(err));
                return NULL;
            }
        ...
        max_count--;
        err = pthread_cond_signal(&condvar);
        if (err) {
            printf("reader: pthread_cond_signal() failed: %s\n", strerror(err));
            return NULL;
        }
...
void *writer(void *arg) {
	...
	while (1) {
        ...
        while(max_count == 1000){
            err = pthread_cond_wait(&condvar, &lock);
            if (err) {
                printf("writer: pthread_cond_wait() failed: %s\n", strerror(err));
                return NULL;
            }
...
int main() {
    ...
    if (err) {
        printf("main: pthread_cond_init() failed: %s\n", strerror(err));
        return -1;
    }
